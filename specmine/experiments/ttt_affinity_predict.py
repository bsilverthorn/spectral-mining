import specmine.experiments.ttt_affinity_predict

if __name__ == "__main__":
    specmine.script(specmine.experiments.ttt_affinity_predict.main)
...
def run_affinity_features(B, vectors_ND, affinity_NN, index, values):
    if B > 0:
    ...
    return run_features("affinity", B, all_features_NF, index, values)
...
def main(out_path, values_path, neighbors = 8, workers = 0):
    ...
    def yield_jobs():
        for B in numpy.r_[0:100:16j].astype(int):
            yield (run_affinity_features, [B, vectors_ND, affinity_NN, index, values])
            yield (run_random_features, [B, vectors_ND, index, values])

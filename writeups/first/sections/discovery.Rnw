...
\section{Spectral Representation Discovery}

XXX

\subsection{Domain 1: Two-Room Grid World}
...
\subsection{Domain 2: Tic-Tac-Toe}

\begin{figure}
<<vs.tabular.first,fig=T,height=4,sanitize=T>>=
data <- read.csv("../results/learning_curve.200games.alpha=0.001.gpe=200.csv")

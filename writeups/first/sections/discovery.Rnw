...
As can be seen in \label{fig:grid.world.evs} the Laplacian eigenvectors form 
interesting functions across the rooms that capture many of the domain's 
symmetries and are adapted to the connectivity of the states. Linear 
combinations of these functions would obviously be useful for approximating 
smooth functions in the two-room domain. This visual example motivates our use 
of the eigenvectors of the graph Laplacian in other domains with rich topology, 
such as game state graphs.
...
separates the two rooms; the third and fourth are (nearly) zero in one room 
while partitioning the other room; and the fifth eigenvector separates the 
doorway from the corners of the room. }
\end{figure}

\section{Tic-Tac-Toe Domain}

With the goal of scaling up to Go, we chose TicTacToe as our second toy domain
as it has many similar properties including a similar tree-like state space 
and discrete board representation. It is also small enough to be able to build
an optimal player, solve for the optimal value function, and find the 
Laplacian eigenvalues on the full state-space graph.

In the following sections, the state values are defined as 1 for all winning
states, -1 for all losing states, and 0 for draws. Similarly, the two players, 
as well as their pieces on the board are denoted by 1 and -1 with zero being an
empty space. $x_{ij}$ refers to the value of board $x$ in the $i^{th}$ row and 
the $j^{th}$ column.

\subsection{Mining the Gameplay Graph}

When choosing a graph on which to perform spectral analysis for representation
discovery in TicTacToe, the simplest and most obvious option is the adjacency 
graph for neighboring board states. We define the gameplay adjacency graph as 
having weight one edges between each board and all previous or subsequent boards.
created undirected edges with weight one between each board. 
...
\begin{figure}
<<ttt.action,fig=T,height=3,sanitize=T>>=

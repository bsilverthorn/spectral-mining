...
representation discovery in TTT, the simplest and most obvious option is the
adjacency graph for neighboring board states. We define the gameplay adjacency
graph, displayed in \cref{fig:ttt.gameplay.graph}, as having weight-one edges
between each board and all previous or subsequent boards.
...
the first nine eigenvectors is given in \cref{fig:ttt.evs}. Again,
the eigenvectors capture much of domain's symmetry. They also provide useful
features for evaluating gameplay, such as separating corner versus center
moves. 
...
between states. This simplification is convenient, since a symmetric matrix is
necessary for computing the eigenvectors, but alternatives such as the directed
graph Laplacian \citep{chung2005laplacians} exist.
...
\emph{affinity graph}, represents information differently than the gameplay
...
%
\begin{itemize}
\item computing the true value function against the optimal opponent using
value iteration;
